...
if [[ "${DATAPOINT_FORM_PATH:0:1}" == "~" ]];
then
	DATAPOINT_FORM_PATH="$HOME${DATAPOINT_FORM_PATH:1}"
fi

cp "$DATAPOINT_FORM_PATH" $TMP_FILEPATH_1

help_function(){
	# This is the help for the script
	echo "This script provides different ways to add a new datapoint to the database."
	echo "The first argument has to be -l (to make the edits in the local terminal) or -t (to edit in zenity and a popup-terminal)"
	echo "The second argument is either a path to a file containing the content, -c (to copy the content from the local clipboard) or nothing to manually enter the content in a terminal."
}

open_terminal(){
	FORM_DATA=$(zenity --forms --width=500 --title="Datapoint information" \
		--text="Please enter the data for your datapoint entry." \
		--add-entry="Citekey"  \
		--add-entry="Page" \
		--add-entry="Summary" \
		--add-entry="Tag 1" \
	...
	SUMMARY=$(echo $FORM_DATA | awk -F '|' '{printf $3}')
	TAG_1=$(echo $FORM_DATA | awk -F '|' '{printf $4}')
	TAG_2=$(echo $FORM_DATA | awk -F '|' '{printf $5}')
	TAG_3=$(echo $FORM_DATA | awk -F '|' '{printf $6}')
	TAG_4=$(echo $FORM_DATA | awk -F '|' '{printf $7}')
	TAG_5=$(echo $FORM_DATA | awk -F '|' '{printf $8}')
	...
	sed -i "s|SUMMARY=|SUMMARY=$SUMMARY|g" $TMP_FILEPATH_1
...
open_local(){
	# Procedure for adding a new datapoint in the local terminal
	if [[ $1 == "-c" ]];
	then
		# Adding the content from the system clipboard
